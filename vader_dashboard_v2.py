...
def formula_source_label(source_column: str) -> str:
    if is_custom_expression_column(source_column):
        return "Y"
...
def validate_custom_expression(expression: str) -> str:
    ...
    if not text:
        raise ValueError("The custom formula cannot be empty.")
    try:
        tree = ast.parse(text, mode="eval")
    except SyntaxError as exc:
        raise ValueError("The custom formula has invalid syntax.") from exc
    ...
    def validate(node: ast.AST) -> None:
        if isinstance(node, ast.Expression):
            ...
            raise ValueError("The custom formula contains an unsupported operation.")
...
def evaluate_custom_expression(
    ...
    if "pi" in referenced_names:
        namespace["pi"] = np.pi
    if "ST" in referenced_names:
    ...
    if calculated.ndim == 0:
        ...
        try:
            ...
            raise ValueError("The custom formula did not produce one value per row.") from exc
...
def analyze_postprocessing_runs(
    ...
    analysis_frame = add_custom_expression_column(
        processed.frame, y_column, physical_settings
    )
...
def _custom_formula_key(scope: str, suffix: str) -> str:
    return f"{scope}_custom_y_{suffix}"


def custom_expression_options(
    options: list[str] | tuple[str, ...],
    state_key: str,
) -> list[str]:
    result = list(options)
    current = str(st.session_state.get(state_key, ""))
    if is_custom_expression_column(current) and current not in result:
        result.append(current)
    return result


def open_custom_y_formula(scope: str, target_key: str) -> None:
    current = str(st.session_state.get(target_key, ""))
    draft_key = _custom_formula_key(scope, "draft")
    ...
    st.session_state[_custom_formula_key(scope, "error")] = ""
    st.session_state["custom_y_dialog_scope"] = scope
...
def apply_custom_y_formula(scope: str, target_key: str) -> bool:
    draft_key = _custom_formula_key(scope, "draft")
    error_key = _custom_formula_key(scope, "error")
    try:
        column = custom_expression_column(st.session_state.get(draft_key, ""))
    except ValueError as exc:
        st.session_state[error_key] = str(exc)
        return False
    st.session_state[target_key] = column
    st.session_state[error_key] = ""
    st.session_state["custom_y_dialog_scope"] = None
    return True


def append_custom_formula_text(
    expression: str,
    token: str,
    token_kind: str,
) -> str:
    current = str(expression).rstrip()
    if token_kind == "symbol":
        ...
        if token == ",":
            return f"{current}, "
        return f"{current} {token} " if current else f"{token} "
    separator = "" if not current or current.endswith("(") else " "
    return f"{current}{separator}{token}"


def append_custom_formula_token(scope: str, token_kind: str) -> None:
    draft_key = _custom_formula_key(scope, "draft")
    token = str(st.session_state[_custom_formula_key(scope, token_kind)])
    st.session_state[draft_key] = append_custom_formula_text(
        str(st.session_state.get(draft_key, "")), token, token_kind
    )


def wrap_custom_y_formula(scope: str) -> None:
    draft_key = _custom_formula_key(scope, "draft")
    function = str(st.session_state[_custom_formula_key(scope, "function")])
    current = str(st.session_state.get(draft_key, "")).strip() or "A / D"
    st.session_state[draft_key] = f"{function}({current})"


@st.dialog("Custom formula", width="large")
def render_custom_y_formula_dialog(scope: str, target_key: str) -> None:
    draft_key = _custom_formula_key(scope, "draft")
    st.session_state.setdefault(draft_key, "A / D")
    builder = st.columns(
        [1.25, 0.25, 0.82, 0.25, 1.0, 0.55],
        gap="small",
    ...
    with builder[0]:
        st.selectbox(
            ...
            key=_custom_formula_key(scope, "variable"),
        )
    with builder[1]:
        st.button(
            "",
            icon=":material/add:",
            help="Insert selected variable",
            key=_custom_formula_key(scope, "insert_variable"),
            on_click=append_custom_formula_token,
            args=(scope, "variable"),
            width="stretch",
        )
    with builder[2]:
        st.selectbox(
            "Symbol",
            CUSTOM_EXPRESSION_SYMBOLS,
            key=_custom_formula_key(scope, "symbol"),
        )
    with builder[3]:
        st.button(
            "",
            icon=":material/add:",
            help="Insert selected symbol",
            key=_custom_formula_key(scope, "insert_symbol"),
            on_click=append_custom_formula_token,
            args=(scope, "symbol"),
            width="stretch",
        )
    with builder[4]:
        st.selectbox(
            ...
            key=_custom_formula_key(scope, "function"),
        )
    with builder[5]:
        st.button(
            "Wrap",
            key=_custom_formula_key(scope, "wrap"),
    ...
    st.text_area(
        ...
        height=110,
        help=(
            "Type directly or insert the same variable as many times as needed. "
            "Symbols: t, Lv, HS, eps_z, D, D0, F, A, sig, sig_surf, "
            "delta_sig, HS_rate, visc_e, v, pi, ST. ST uses mN/m."
        ),
    )
    error = str(st.session_state.get(_custom_formula_key(scope, "error"), ""))
    if error:
        st.error(error)
    ...
    apply_clicked = actions[0].button(
        "Apply",
        type="primary",
        key=_custom_formula_key(scope, "apply"),
        width="stretch",
    )
    cancel_clicked = actions[1].button(
        "Cancel",
        key=_custom_formula_key(scope, "cancel"),
        width="stretch",
    )
    if apply_clicked:
        if apply_custom_y_formula(scope, target_key):
            st.rerun()
...
def render_custom_formula_button(scope: str, target_key: str) -> None:
    st.button(
        "",
        icon=":material/functions:",
        key=f"{scope}_open_custom_y",
        help="Define a custom formula",
        on_click=open_custom_y_formula,
        args=(scope, target_key),
        width="stretch",
    )
    if st.session_state.get("custom_y_dialog_scope") == scope:
        render_custom_y_formula_dialog(scope, target_key)
...
def render_axis_selector(
    ...
    options = custom_expression_options(axis_options, state_key)
    ...
    if custom_y_enabled:
        with row[2]:
            render_custom_formula_button(key_prefix, state_key)
    return str(selected)
...
def render_processing_controls(
    ...
    formula_source = canonical_formula_source(formula_source_label(source_column))
    if formula_key not in st.session_state:
        st.session_state[formula_key] = (
            default_formula or formula_source_label(source_column)
        )
    ...
    try:
        ...
        formula_valid = parsed_source == formula_source
    except ValueError:
        current_steps = ()
        formula_valid = False
    ...
    with st.popover(button_label, width="stretch"):
        formula = st.text_input(
            ...
            help=(
                ...
                "The innermost value must match the selected signal."
            ),
        )
        try:
            ...
            if not formula_valid:
                st.error(f"Formula must start from {formula_source_label(source_column)}.")
            else:
        ...
        actions[0].button(
            ...
            args=(formula_key, formula_source, "add", new_step),
        )
        actions[1].button(
            "Remove",
            icon=":material/undo:",
            ...
            on_click=update_filter_formula,
            args=(formula_key, formula_source, "remove", None),
        )
        actions[2].button(
            "Reset",
            icon=":material/restart_alt:",
            ...
            on_click=update_filter_formula,
            args=(formula_key, formula_source, "reset", None),
        )
...
def render_postprocessing_workspace(
    ...
    with st.container(border=True, key="postprocessing_controls_v5"):
        ...
        with control_columns[3]:
            y_label, y_control, custom_control = st.columns(
                [0.14, 0.68, 0.18], gap=None, vertical_alignment="center"
            )
            with y_label:
            ...
            with y_control:
                y_column = st.selectbox(
                    ...
                    custom_expression_options(
                        PROCESSED_AXIS_COLUMNS, "postprocessing_y"
                    ),
                    index=None,
            ...
            with custom_control:
                render_custom_formula_button(
                    "postprocessing", "postprocessing_y"
                )
...
def render_frequency_workspace(
    ...
    with st.container(border=True, key="frequency_toolbar_separate_v3"):
        ...
        with control_columns[0]:
            variable_label, variable_control, custom_control = st.columns(
                [0.32, 0.52, 0.16], gap=None, vertical_alignment="center"
            )
            with variable_label:
            ...
            with variable_control:
                signal_column = st.selectbox(
                    "Signal",
                    custom_expression_options(
                        AVAILABLE_COLUMNS, "frequency_signal"
                    ),
                    index=None,
    ...
    data = derive_dataset(preprocessed.frame, applied_physics)
    try:
        data = add_custom_expression_column(
            data, applied_request.signal_column, applied_physics
        )
    except ValueError as exc:
        frequency_status.error(f"Custom formula failed: {exc}")
        return
    labels = {
        row["source_file"]: get_file_label(row)
        for _, row in file_summary.iterrows()
...
def spectrum_axis_title(kind: str, signal_column: str) -> str:
    if is_custom_expression_column(signal_column):
        symbol = custom_expression_text(signal_column)
        if kind == "FFT":
            return f"|FFT({symbol})|"
        if kind == "PSD":
            return f"PSD({symbol})"
        return f"<i>E</i>({symbol})"
