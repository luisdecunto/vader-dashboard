...
def render_plot_window(
    ...
    with st.container(border=True):
        ...
        if allow_processing:
            toolbar = st.columns(
                [0.35, 1.45, 0.35, 1.45, 1.05, 1.05, 1.05, 1.05, 0.8],
                gap="small",
        ...
        if not selected_files:
            st.info("Select data series to load and plot.")
            return
        ...
        for issue in issues:
            st.warning(issue)
        if raw_data.empty:
            st.info("No compatible rows were loaded for the selected data series.")
            return
        ...
        raw_frame = derive_dataset(preprocessed.frame, physical_settings)
        ...
        job_key = make_job_key(
            scope, index, signature, physical_settings,
        ...
        render_background_processed_plot(
            ...
            force_variants, strain_variants, physical_settings, show_raw_overlay,
...
def render_summary_workspace(
    ...
    materials = sorted(file_summary["material"].dropna().unique().tolist())
    velocities = sorted(file_summary["velocity"].dropna().unique().tolist())
    filter_columns = st.columns([1.0, 1.0, 0.55], gap="small")
    ...
    if not selected_files:
        st.info("Select data series to build summary plots.")
        return
    raw_data, issues = load_selected_dataset(
        str(DATA_DIR), signature, tuple(selected_files)
    )
...
def render_frequency_workspace(
    ...
    if not selected_files:
        st.info("Select one or more data series to run frequency analysis.")
        return

    controls = st.columns([1.5, 1.0], gap="small")
    with controls[0]:
    ...
    job_key = make_job_key(
        "frequency-batch", signature, physical_settings, filter_settings,
        tuple(selected_files), signal_column, peak_settings,
    )
    render_background_frequency(
        job_key, data, signal_column, filter_settings, peak_settings, labels
    )
...
def render_background_frequency(
    ...
    analysis_settings = peak_settings[:4]
    future = submit_background_job(
        job_key,
    ...
    if future.done():
        ...
        render_frequency_results(
            result, signal_column, series_labels, peak_settings
        )
        return
...
def render_frequency_results(
    ...
    peak_min_hz, peak_max_hz, peak_count, _, histogram_bins = peak_settings
    sample_rates = [result.sample_rate_hz for result in batch.results.values()]
    st.caption(
        f"{len(batch.results)} run(s) | Sampling rate: "
    ...
    for index, (source_file, result) in enumerate(batch.results.items()):
        ...
        psd_figure.add_trace(go.Scatter(
        ...
        energy_figure.add_trace(go.Scatter(
            x=centers,
            y=result.energy,
            mode="lines+markers",
        ...
        warnings.extend(
            f"{label}: {warning}" for warning in result.warnings
        )
    ...
    configure_analysis_figure(
        energy_figure, "Frequency band (Hz)", "Integrated PSD energy", 330
    )
    for figure in (fft_figure, psd_figure, energy_figure):
        figure.update_layout(showlegend=show_legend, legend=legend)
        figure.update_xaxes(range=[peak_min_hz, peak_max_hz])
    ...
    if not peak_table.empty:
        ...
        histogram_figure.add_trace(go.Histogram(
            ...
            marker={"color": "#7559a6"},
            hovertemplate="Frequency: %{x:.4g} Hz<br>Count: %{y}<extra></extra>",
        ))
    configure_analysis_figure(
        histogram_figure, "Peak frequency (Hz)", "Peak count", 330
    )
    histogram_figure.update_xaxes(range=[peak_min_hz, peak_max_hz])

    lower_columns = st.columns(2, gap="small")
    with lower_columns[0]:
        st.plotly_chart(energy_figure, width="stretch", config=plotly_config())
    with lower_columns[1]:
        st.plotly_chart(histogram_figure, width="stretch", config=plotly_config())

    st.markdown(
        '<div class="plot-heading">Strongest detected peaks by run</div>',
        unsafe_allow_html=True,
    )
    if peak_table.empty:
        st.info("No peaks were detected in the selected frequency range.")
    else:
